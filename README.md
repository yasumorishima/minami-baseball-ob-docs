...
マスターズ甲子園への挑戦を続ける横浜市立南高校 野球部OB会の公式Webアプリケーション。試合結果・予定管理、1955年からの歴代戦績<!--stat:senseki-->686<!--/stat-->試合のデータベース、会員管理、写真ギャラリーなどを、5段階の権限制御のもとで運用しています。
...
| Metric | Count |
|--------|-------|
| TypeScript/TSX files | <!--stat:ts_files-->157<!--/stat--> |
| Lines of code | <!--stat:loc-->~18200<!--/stat--> |
| Page routes | <!--stat:pages-->44<!--/stat--> |
...
- **雨アイコン 4 段階**: 単一 rain icon を 小雨 (2 短いしずく) / 中雨 (3 標準しずく = 旧と同じ) / 大雨 (4 太く長いしずく) / 暴風雨 (斜めしずく + 風流線) に分割。 mm prop 優先 + WMO code fallback で判定。 daily 閾値 = JMA 注意報 30/警報 80 mm/日 + 5 mm/日 で小→中、 hourly 閾値 = JMA 雨の強さ 目安 1/5/15 mm/時。 「降水 60%」 が同じ確率でも 1mm と 50mm でアイコンが変わる
- **Bi-directional weatherCode ↔ probability reconciliation**: Open-Meteo computes `weatherCode` and `precipitation_probability` independently and sometimes returns contradictions (e.g. `code=3 cloudy + prob=98%` or `code=51 drizzle + prob=3%`). `getEffectiveWeatherCode` reconciles both directions so the displayed weather icon and the umbrella indicator always agree — light rain codes at <20% are downgraded to cloudy, clear/cloudy at ≥20%/≥80% are upgraded to light rain/rain **only when actual forecast precipitation is present (precipitationMm ≥ 0.1mm)** — probability alone no longer forces a rain icon, since Open-Meteo often returns clear code + high probability with 0mm (which previously caused phantom rain icons and a high precipitation % on sunny days). The displayed daily precipitation % is likewise computed only from hours with mm ≥ 0.1. Severe codes (heavy rain 65, violent showers 82, freezing rain 56-57/66-67, snow 71-77/85-86, thunder 95+) are always preserved and always show their warning icon regardless of probability, so rare but critical conditions are never hidden
...
Supabase `history_matches` テーブルに<!--stat:senseki-->686<!--/stat-->試合分の戦績データを格納。FC2旧サイトからのパース、外部ソースとの突合検証を経て構築。サイト上からeditor以上が直接編集可能。

- <!--stat:senseki-->686<!--/stat--> games in `history_matches` table with DB sequence for auto-ID
...
- Dynamic sitemap generation for all <!--stat:senseki-->686<!--/stat--> game detail pages (with `lastModified` from `updated_at`)
...
- **Current team game detection (PR-review flow)**: Automated scraping from 3 sources (kyureki.com + hb-nippon.com + baseball-memories.jp) detects new games. The first two carry only the prefecture-level tournament; district qualifiers (地区予選) appear on none of them, so baseball-memories.jp is walked from the school's stable category page to each block page and the plain-text result lines are parsed. A guard requires 「南」 to actually be a participant and drops site-navigation strings as opponents. Detected games are written to `data/pending-history.json` and proposed as a **pull request** for human review — merging the PR triggers insertion into Supabase `history_matches` (idempotent). Changed 2026-07 from the previous direct-insert flow after a mis-parsed heading string was recorded as an opponent for a game 南 did not play. Runs on `ubuntu-latest` (2026-08-17), moving back to the RPi5 self-hosted runner only if hosted minutes run out.
...
<!--stat:tables_main-->17<!--/stat--> tables + <!--stat:tables_hist-->6<!--/stat--> history tables + 4 views, all protected by Row-Level Security.
...
| **Health Check** (public-cron, monitoring) | Hourly (`7 * * * *`) on minami-public-cron | 4 probe + ワークフロー実行監視を hourly で実行: ① Vercel proxy `/api/gas-proxy/dispatch` 200 必須 ② dispatch chain → ack workflow 5min 以内 ③ GAS time trigger gas-heartbeat 鮮度 2.16h 以内 ④ gas-issue-form Web App secret 一致 (feedback Gmail path。 deprecated webapp の Google 側 transient による誤発報を避けるため **3 回リトライ + 単発 timeout/接続失敗/非200 は warning のみで job を落とさず**、 secret 不一致 `unauthorized` のみ hard fail) ⑤ `check-pipeline-health.py` が member-request / sync-roles のワークフロー実行失敗と sync-roles liveness (cron 停止=6h 以上未実行、GitHub の scheduled cron 間引き ~4h を考慮した閾値) を検出 → GitHub issue 自動作成 + GAS Gmail 通知、復旧で自動 close。 失敗時 GHA standard email + GAS GmailApp の二系統 alert。 2026-04-20 〜 05-24 (1 ヶ月超 silent fail) 事案の再発防止 |
...
| **Check Current Team** | Daily during the season (Apr-Nov, JST 19:00) / manual | Scrapes kyureki.com + hb-nippon.com + baseball-memories.jp for new games (南-participant guard), writes candidates to `data/pending-history.json` and opens a **PR** for review. Merging triggers **Apply Pending History** to insert (idempotent); a run that finds nothing closes a stale candidate PR. The `minami-public-cron` copy stays disabled so PRs are not opened twice. |
...
| **Daily Message** | (移行先: [minami-public-cron](https://github.com/yasumorishima/minami-public-cron) で ubuntu-latest 動作、 旧 self-hosted 版は disabled) `*/30 * * * *` polling + JST 時刻から slot 自動判定 / manual | Generates greeting messages via Gemini 2.5 Flash + weather. 30 分 polling で各 slot (morning 06-12 / noon 12-18 / night 18-24 JST) を自動判定、 API 冪等性で既存は HTTP 200 skipped。 各 slot 最終 30 分 (11:30/17:30/23:30 JST) は `is_backfill=true` で失敗時 email 通知 |
...
| **Silent-fail monitoring** | 会員申請 + フィードバックパイプライン全段 (Vercel proxy / dispatch chain / GAS time trigger / gas-issue-form secret / sync-roles ワークフロー失敗 + liveness) を minami-public-cron の `health-check.yml` + `check-pipeline-health.py` で hourly 検査 (異常は GitHub issue 自動作成・復旧で自動 close) + GAS `hourlyHealthCheck` から内側 probe (Vercel heartbeat + gas-issue-form secret、 毎時実行をまたいだ連続失敗カウンタ `evaluateProbe_` で debounce = 単発の Google/Vercel transient は次の毎時実行で自己回復するため無音。 Vercel heartbeat は連続 2 回 ≒2 時間継続で初めて通知。 gas-issue-form は外側 `health-check.yml` と同方針で、 secret 不一致 `unauthorized` は即通知 / Google 側が返す Docs 系 HTML シェル・404 は admin に対処不能な一過性として無音 (≒6 時間継続した時だけ通知)、 失敗種別ごとにカウンタを分離して取りこぼしを防ぐ)、 失敗時は GHA failure email + GAS GmailApp の二系統で admin alert。 2026-04-20 の `appsscript.json` oauthScope 追加で trigger OAuth 失効 → form 申請 1 ヶ月超 silent fail した事案を受け、 二度と気付かないことが起きないよう外側 + 内側 + 二系統通知の三重防御 |
| **Dependency vulnerability management** | 2026-06-10 全数監査: 本番依存 70 パッケージを OSV.dev (Google 脆弱性DB) で照会し、next 15.5.14 の既知脆弱性 14 件 (HIGH: middleware/proxy バイパス GHSA-267c-6grr-h53f / GHSA-26hh-7cqf-hhc6 / GHSA-492v-c6pp-mqqv、DoS、SSRF ほか) を検出 → 即日 next 15.5.18 / ws 8.21.0 へ bump。本サイトは /admin・/edit・/members-only の認可を middleware で行うためバイパス系は直撃構成だった (データ自体は RLS の defense-in-depth で保護)。残存は next 内部 pin の postcss 8.4.31 のみ (moderate・ビルド時のみ使用で実行時露出なし・最新 next も同 pin)。2026-07-27、high 4 系統を解消: **sharp 0.35.3** (GHSA-f88m-g3jw-g9cj = libvips CVE-2026-33327/33328/35590/35591。sharp は直接依存していないが next が同梱する 0.34.5 が対象のため `overrides` で巻き取り)、**next 15.5.22** (SSRF・DoS・cache confusion 等 8 勧告が <15.5.21 対象。16 系はメジャーのため据え置き)、**postcss 8.5.23** (旧 override pin ^8.5.10 が新勧告 <=8.5.17 の範囲に入っていた＝過去に潰した override も再検証が要る)、**brace-expansion 1.1.16**。`npm audit --omit=dev` = 0 / Dependabot open = 0、Vercel deploy も success。残る brace-expansion の勧告 (範囲 <=5.0.7 で 1.x 系も semver 上は該当) は lint 専用で本番バンドルに載らず、eslint-plugin-import / react / jsx-a11y が最新版でも minimatch@^3 依存のため上流に修正版が存在しない (minimatch を上げると CJS export が関数でなくなり plugin が壊れる)。glob パターンは自前の eslint 設定由来で攻撃者の制御下にないため tolerable_risk として dismiss、上流が minimatch を上げたら再評価 |
| **Live-tested access control** | 2026-06-10 実環境検証: SET ROLE anon シミュレーションで members / user_roles / dues_payments は 0 行 (announcements 等の公開テーブルのみ可視)、匿名アクセスで /members-only・/admin・/edit は 307 → /login、管理系 API は 401、全テーブルの INSERT/UPDATE/DELETE は editor+ の WITH CHECK 付き RLS を確認。storage は documents / members-docs が private、photos / videos のみ public (設計通り) |
| **Feedback image privacy** | 2026-06-11: feedback 添付画像の保存先を public repo から **private Supabase Storage bucket `feedback-images`** + 10 年署名付き URL へ移行 (feedback route と GAS proxy upload-image の両経路、応答形式維持で GAS 側変更不要)。ファイル名はタイムスタンプ+乱数+ASCII 化で投稿者名を含まない。E2E 検証済 (署名 URL 200 / token 無し 400 / public repo への新規追加ゼロ) |
...
Public (17 pages)
  ...
  /history                 Historical records 1955-2026 (<!--stat:senseki-->686<!--/stat--> games)
...
| Icons | Custom pixel art mascots (pitcher, batter, fielder) + いらすとや baseball mascot (daily-message avatar) |
...
- **Data migrated**: 歴代戦績 <!--stat:senseki-->686<!--/stat-->試合 (1955-2026) -> Supabase `history_matches` table (editable by editor+)
...
- 本サイトはOB有志が**非営利・無償のボランティア**で制作・運営しています。専門の業者ではありません。
- 通常であればこの規模のサイトは制作会社への委託費や毎月のサーバー費用が発生しますが、**費用ゼロで構築・運用**しています。無料で公開し続けられているのは技術的な工夫の積み重ねによるもので、当たり前に回っているわけではありません。維持そのものにも相応の時間と専門的な手間がかかっています。
- そのため、**機能追加のご依頼は原則としてお受けしていません**。不具合の報告・情報提供は歓迎します。技術に明るい方がご自身で改修された **Pull Request** も歓迎します。
- 非営利運営のため、**有償のご依頼もお受けしていません**。
- 大幅な機能追加・作り直しは個人では対応しきれません。本 README に技術構成を公開しているので、**引き継ぎ・再構築を検討される際の参考**にしてください。
