...
### 公開ページ
- **トップページ** — ヒーロー画像（CTAボタン付き） + はじめての方へ紹介セクション + 活動紹介パネル + 活動の様子（ランダム写真） + 新着情報 + 今後の予定 + OB会戦績
...
- **お知らせ** — 写真付きの告知
- **現役野球部** — 最新情報 + 歴代戦績への導線
- **ギャラリー** — 紐付け先別フォルダ表示（折りたたみ式）、YouTube埋め込み再生、写真クリックでライトボックス表示（モーダル拡大・左右矢印・キーボード操作対応）
- **グローバル検索** — ヘッダーの検索アイコンから試合結果・予定・お知らせ・歴代戦績を横断検索（デバウンス付き、全結果がクリックで詳細ページにリンク）
- **プライバシーポリシー** — Cookie同意・データ削除方法の記載
- **カスタム404ページ**
- **エラーバウンダリ** — ランタイムエラー時に独自UI表示（error.tsx / global-error.tsx） — ドット絵マスコット「空振り！」
- **PWAインストールバナー** — iOS Safari（共有→ホーム画面に追加の手順案内）/ Android Chrome（beforeinstallpromptでネイティブインストール）対応。Cookie同意バナーとの表示競合回避。一度閉じたら再表示しない

### 会員専用ページ（member権限以上）
- 登録メンバー一覧（卒業期・名前・権限、折りたたみ表示）
- OB会費納入状況（年度別折りたたみ、済/未トグル）
- **お気に入り一覧** — ブックマークした記事を一覧表示（未ログイン時はログイン案内、歴代戦績は対戦相手・スコア表示、空状態に使い方説明）

### 編集機能（editor権限以上）
- 予定・試合結果・お知らせ・現役情報の追加・編集・削除
- **編集ボタン**: 全7リストページ（試合結果/予定/お知らせ/現役野球部/歴代戦績/マスターズ/ギャラリー）にeditor以上向け編集ページリンク
...
- **ダークモード対応**: ヘッダーの切替ボタン（ライト/ダーク表示）で明暗切替。モバイルでも常時表示
...
- **ブックマーク**: ログインユーザーが記事をお気に入り保存（全詳細ページにアイコン+テキストラベル付きボタン）
- **レスポンシブヘッダー**: モバイルはハンバーガーメニュー（ラベル付き展開）、デスクトップはアイコン+テキスト横並び
- **勝敗テキスト表示**: ResultBadgeに「○ 勝ち / ● 負け / △ 引分」テキスト併記
- **スコアにチーム名**: 試合詳細の得点表示に「南高OB」「相手チーム名」ラベル付き
- **写真ライトボックス**: 全詳細ページ+ギャラリーで写真をモーダル拡大表示（左右矢印・キーボード操作・枚数表示）
- **PWAインストールバナー**: ホーム画面追加の案内。iOS Safari（共有アイコン+手順案内）/ Android Chrome（ネイティブインストールプロンプト）。Cookie同意バナー表示中は待機、一度閉じたら再表示しない
- **初見ユーザー向けUX**: ホームに「はじめての方へ」紹介セクション、ヒーローにCTAボタン（「マスターズ甲子園への挑戦」+「OB会について」）、お知らせ・予定行のクリッカブル化（タップヒント「›」付き）
...
- **写真ライトボックス**: 全公開ページの写真をクリックでモーダル拡大表示（左右矢印・キーボード操作・枚数表示対応）
...
- **移管済み**: OB会概要・設立趣旨、活動内容、校歌・応援歌、関連リンク、注意書き、OB会規約（全15条+付則+振込先）
