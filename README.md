...
- **Daily greeting (朝/昼/夜のひとこと)**: Gemini 2.5 Flash + Grounded Web Search で 1日3回（JST 6:00/12:00/18:00、GitHub Actions cron）、横浜スタジアム座標の Open-Meteo 実天気 + その日の記念日を元に 80字前後の挨拶メッセージを自動生成し `daily_messages` テーブルに保存。トップページは現在時刻からスロット判定（朝6-12/昼12-18/夜18-6）。冪等チェック + 日次サーキットブレーカー10回 + `CRON_SECRET` Bearer 認証で多層防御。Gemini キーは billing 無効の GCP プロジェクトに restrict され無料枠超過時は 429（課金不可）。プロ野球ニュースなど不確実な情報は禁止、歴史上の記念日のみ許可、スロット別トーン指示（朝=未来形/昼=現在進行/夜=完了形）。**自動回復（二段構え）**: ①API 内 retry 4 試行 + 指数バックオフ（1s/2s/4s）で同 run 内に Gemini 一過性 5xx を吸収、Vercel `maxDuration = 60` を明示しワーストケース ~50s が Hobby 上限内に収まるよう設定、curl `--max-time 90s`。②それでも失敗時は各スロットの 3 時間後（JST 09:00/15:00/21:00）に補完 cron が拾う。スロット判定は `github.event.schedule` ベースで遅延実行でも正確。 **UI**: 左に baseball mascot avatar (いらすとや、 全身 illustration を rounded circle 内に object-contain で fit、 `/public/mascot-baseball.png`)、 右に speech bubble card (border + 2-layer triangle tail、 `bg-card`) で「誰かが話してる」 風表示。 home の他 card と構造で差別化 (色ではなく shape で identifiable)
