...
| **Check Current Team** | Daily during the season (Apr-Nov, JST 19:00) / manual | Scrapes kyureki.com + hb-nippon.com for new games (南-participant guard), writes candidates to `data/pending-history.json` and opens a **PR** for review. Merging triggers **Apply Pending History** to insert (idempotent); a run that finds nothing closes a stale candidate PR. The `minami-public-cron` copy stays disabled so PRs are not opened twice. |
