...
- **歴代戦績** — 現役野球部の戦績（1955〜2026年、680試合）。年別折りたたみ + 大会種別フィルタ + 写真付き試合はアイコン表示
- **今後の予定** — 試合・練習・懇親会など（種類別バッジ表示）
- **お知らせ** — 写真付きの告知
- **現役野球部** — 最新情報 + 歴代戦績への導線
- **ギャラリー** — 紐付け先別フォルダ表示（折りたたみ式）、YouTube埋め込み再生、写真クリックで別タブに原寸表示
- **グローバル検索** — ヘッダーの検索アイコンから試合結果・予定・お知らせ・歴代戦績を横断検索（デバウンス付き）
- **プライバシーポリシー** — Cookie同意・データ削除方法の記載
- **カスタム404ページ**
- **エラーバウンダリ** — ランタイムエラー時に独自UI表示（error.tsx / global-error.tsx） — ドット絵マスコット「空振り！」

### 会員専用ページ（member権限以上）
- 登録メンバー一覧（卒業期・名前・権限、折りたたみ表示）
- OB会費納入状況（年度別折りたたみ、済/未トグル）
- **お気に入り一覧** — ブックマークした記事を一覧表示
...
### 管理機能（admin権限のみ）
- 権限設定（viewer/member/editor の切替）
- ゴミ箱管理（`/admin/trash` — 全テーブルの削除済みレコードを一覧・復元・完全削除）
- **操作ログ**（`/admin/audit` — 権限変更・削除操作の監査ログ。ユーザー名表示+ページネーション）
- サイト設定（外部サービスリンク集: GitHub / Vercel / Supabase / GA4 / Googleフォーム / 技術ドキュメント）
...
- **Cookie同意バナー**: GA4のCookieを同意後のみ読み込み。アカウント設定からリセット可能
- **パンくずリスト**: 全ページに表示（トップ / 一覧 / 詳細 の階層ナビ）
- **トップに戻るボタン**: スクロール400px超でフローティングボタン表示
- **セッションタイムアウト**: 60分操作なしで自動ログアウト（5分前に警告モーダル、クロスタブ同期）
- **最終編集者・日時表示**: 全詳細ページに更新者名と更新日を表示
- **ブックマーク**: ログインユーザーが記事をお気に入り保存（全詳細ページ対応）
...
| 5 | `admin`（管理者） | 全権限（権限変更・データ削除・履歴閲覧） |
...
member-request.yml → PR自動作成
    新規: 「会員申請: 30期 → member」
    変更: 「権限変更: 30期 member → editor」
    ※個人名はGitに記録せずSupabase DBに直接保存（プライバシー保護）
         ↓
管理者 → PRを確認してマージ
         ↓
...
- アカウント設定でデータエクスポート・Cookie設定リセット・退会が可能
...
| `audit_logs` | 管理操作の監査ログ（権限変更・ソフトデリートをDBトリガーで自動記録。admin閲覧用） |
| `bookmarks` | ブックマーク/お気に入り（ユーザーごとの記事保存。RLSで自分のみ操作可） |
...
| 関数 | 用途 |
|------|------|
| `set_updated_at()` | updated_atを自動更新 |
| `log_user_roles_change()` | user_rolesのINSERT/UPDATE/DELETEをaudit_logsに記録 |
| `log_soft_delete()` | ソフトデリート操作をaudit_logsに記録 |
...
大会資料（PDF等）は [masters-docs](https://github.com/yasumorishima/masters-docs) リポに年度別で保存（サイト上からアップロード → GitHub Contents API 経由）。
...
/search              グローバル検索（公開）
/login               ログイン（公開）
/account             アカウント設定 + データエクスポート + Cookie設定 + 退会（ログイン済み）
/bookmarks           お気に入り一覧（ログイン済み）
/members-only/*      会員専用ページ（member以上）
/edit/*              編集画面 8画面（editor以上）
/admin/*             管理画面 — 権限設定・ゴミ箱・操作ログ・サイト設定（adminのみ）
```
...
- **データ化済み**: 現役野球部の歴代戦績（1955〜2026年、680試合）→ `data/senseki.json`（検証率41%、260試合検証済み）
...
- **Cookie同意**: GA4のCookieは同意後のみ読み込み（GDPR対応）
- **オープンリダイレクト防止**: auth callbackで外部URLへのリダイレクトを遮断
- **セッションタイムアウト**: 60分操作なしで自動ログアウト（クロスタブ同期）
- **アカウント削除**: ユーザー自身が退会可能（auth + user_roles完全削除）
- **データエクスポート**: ユーザーが自分のデータをJSON形式でダウンロード可能
- **構造化ログ**: 全APIルートでJSON形式ログ出力（Vercelダッシュボードで監視可能）
- **監査ログ**: 権限変更・削除操作をDBトリガーで自動記録
...
- **Supabase CLI**: マイグレーションファイルで DB スキーマをバージョン管理（020まで適用済み）
- **GitHub**: `yasumorishima/minami-baseball-ob` で運用（private）
