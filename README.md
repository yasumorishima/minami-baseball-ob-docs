...
### 会員専用ページ（member権限以上）
- 登録メンバー一覧（卒業期・名前・権限をテーブル表示、デスクトップ/モバイル対応）
- 住所録・連絡先など、会員向けの個人情報を今後掲載予定（対外非公開）
...
- **白ベース + エンジ（チームカラー）アクセント**: ヘッダー/ボトムナビにエンジのライン、フッターは暗いエンジ背景
...
```
OBメンバー → Googleフォーム送信（Googleログイン必須）
...
マスターズ甲子園ページは `game_type LIKE 'マスターズ甲子園%' OR game_type = 'ー'` でフィルタ。「ー」は2020以前のマスターズ戦績等、種別が不明な試合に使用。
...
- **データ化済み**: 現役野球部の歴代戦績（1955〜2026年、680試合）→ `data/senseki.json`（検証率38%、260試合検証済み）
- **データ化済み**: マスターズ甲子園過去戦績（2011〜2024年、20試合）→ `results` テーブル（2021年以降は「予選」、2020以前は種別「ー」）
