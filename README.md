...
マスターズ甲子園への挑戦を続ける横浜市立南高校 野球部OB会の公式Webアプリケーション。試合結果・予定管理、1955年からの歴代戦績<!--stat:senseki-->682<!--/stat-->試合のデータベース、会員管理、写真ギャラリーなどを、5段階の権限制御のもとで運用しています。
...
| Metric | Count |
|--------|-------|
| TypeScript/TSX files | <!--stat:ts_files-->149<!--/stat--> |
| Lines of code | <!--stat:loc-->~17300<!--/stat--> |
...
| DB migrations | <!--stat:migrations-->44<!--/stat--> |
| GitHub Actions workflows | <!--stat:workflows-->5<!--/stat--> |
| Historical game records | <!--stat:senseki-->682<!--/stat--> (1955-2026) |
<!-- stats-end -->
...
Supabase `history_matches` テーブルに<!--stat:senseki-->682<!--/stat-->試合分の戦績データを格納。FC2旧サイトからのパース、外部ソースとの突合検証を経て構築。サイト上からeditor以上が直接編集可能。

- <!--stat:senseki-->682<!--/stat--> games in `history_matches` table with DB sequence for auto-ID
...
- Dynamic sitemap generation for all <!--stat:senseki-->682<!--/stat--> game detail pages (with `lastModified` from `updated_at`)
...
- **Current team game detection (PR-review flow)**: Automated scraping from 2 sources (kyureki.com + hb-nippon.com) detects new games. A guard requires 「南」 to actually be a participant and drops site-navigation strings as opponents. Detected games are written to `data/pending-history.json` and proposed as a **pull request** for human review — merging the PR triggers insertion into Supabase `history_matches` (idempotent). Changed 2026-07 from the previous direct-insert flow after a mis-parsed heading string was recorded as an opponent for a game 南 did not play. Runs on the RPi5 self-hosted runner.
...
| **Check Current Team** (RPi5) | Weekly (Monday JST 19:00) / manual | Scrapes kyureki.com + hb-nippon.com for new games (南-participant guard), writes candidates to `data/pending-history.json` and opens a **PR** for review. Merging triggers **Apply Pending History** to insert (idempotent). The `minami-public-cron` copy is disabled (2026-07). |
...
Public (17 pages)
  ...
  /history                 Historical records 1955-2026 (<!--stat:senseki-->682<!--/stat--> games)
...
- **Data migrated**: 歴代戦績 <!--stat:senseki-->682<!--/stat-->試合 (1955-2026) -> Supabase `history_matches` table (editable by editor+)
