...
| Metric | Count |
|--------|-------|
| TypeScript/TSX files | <!--stat:ts_files-->136<!--/stat--> |
| Lines of code | <!--stat:loc-->~16100<!--/stat--> |
| Page routes | <!--stat:pages-->38<!--/stat--> |
| API routes | <!--stat:apis-->13<!--/stat--> |
| Reusable components | <!--stat:components-->45<!--/stat--> |
| DB tables (+ history) | <!--stat:tables_main-->16<!--/stat--> + <!--stat:tables_hist-->6<!--/stat--> |
| DB migrations | <!--stat:migrations-->41<!--/stat--> |
...
- **WBGT (熱中症指数) warning**: 気温と湿度から `0.725*T + 0.0368*RH + 0.00364*T*RH - 4.25` で算出し、日本スポーツ協会の運動指針に沿って 注意(21+)/警戒(25+)/厳重警戒(28+)/危険(31+) を判定。「暑さ指数の目安」パネルで 4 段階の閾値 + 推奨行動（こまめに水分補給を/積極的に休憩・水分補給を/激しい運動は避けてください/運動は原則中止）を /weather と試合詳細天気欄の上部にまとめて 1 つだけ表示し、/weather では sticky-top でスクロール中も視認継続。個別バッジは compact（⚠ ラベル + 暑さ指数 N、℃なしで気温値との混在を回避）。涼しい日（全球場 WBGT < 21）は panel 自体を非表示。屋外練習・試合の熱中症リスクを当日朝に判断できる
